...
class EncarParser:
            ...
            for type_ in param["types"]:
                for body_ in param["body_type"]:
                    ...
                    for page in range(1, pages + 1):
                        ...
                        tasks.append(
                            asyncio.create_task(self.get_cars(data_url, body_))
                        )
                    ...
                    for car in result.values():
                        ...
                        tasks.append(asyncio.create_task(self.download_photo(car)))
                            ...
                            result.append(
                                {
                                    "source": "encar",
                                    "id": int(car["Id"]),
                                    "preview": await self.extract_preview(
                                        car.get("Photos")
                                    ),
                                    "body_type": re.search(
                                        r"\.(?P<type>.+)\.", body_type
    ...
    async def extract_grade(car):
        return f'{car.get("FormDetail")} {car.get("Capacity")} {car.get("Badge")} {car.get("BadgeDetail")}'.replace(
            "None", ""
        ).strip()
